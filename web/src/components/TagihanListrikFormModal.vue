...
watch(
  ...
  (isOpen) => {
    if (isOpen) {
      ...
      form.meter_awal = props.editing?.meter_awal ?? ''
      form.meter_akhir = props.editing?.meter_akhir ?? ''
      form.tarif = props.editing?.tarif ?? 1500
...
const pemakaian = computed(() => {
  const awal = Number(form.meter_awal)
  const akhir = Number(form.meter_akhir)
  return isNaN(awal) || isNaN(akhir) || akhir < awal ? 0 : akhir - awal
})

const estimasiTotal = computed(() => pemakaian.value * Number(form.tarif || 0))
...
function handleSubmit() {
  if (!form.kamar_id || !form.periode || form.meter_awal === '' || form.meter_akhir === '' || !form.tarif || !form.jatuh_tempo) return
  emit('submit', {
    ...form,
    kamar_id: Number(form.kamar_id),
    meter_awal: Number(form.meter_awal),
    meter_akhir: Number(form.meter_akhir),
    ...
    <div class="relative bg-white rounded-card shadow-card w-full max-w-md p-5 lg:p-6 my-auto">
      <div class="flex items-center justify-between mb-5">
        <h2 class="text-[16px] font-semibold text-ink-900">
          {{ editing ? 'Ubah Tagihan Listrik' : 'Tambah Tagihan Listrik' }}
        </h2>
      ...
      <form class="space-y-4" @submit.prevent="handleSubmit">
        ...
        <div class="grid grid-cols-2 gap-3">
          <div>
            <label class="block text-[13px] font-medium text-ink-700 mb-1.5">Meter Awal</label>
            <input
              v-model="form.meter_awal"
              type="number"
              min="0"
              required
          ...
          <div>
            <label class="block text-[13px] font-medium text-ink-700 mb-1.5">Meter Akhir</label>
            <input
              v-model="form.meter_akhir"
              type="number"
              min="0"
              required
              class="w-full rounded-lg border border-ink-100 px-3 py-2.5 text-[13.5px] text-ink-900 focus:border-brand-400 focus:outline-none"
        ...
        <div>
          <label class="block text-[13px] font-medium text-ink-700 mb-1.5">Tarif per kWh (Rp)</label>
          <input
            v-model="form.tarif"
            type="number"
            min="0"
        ...
        <div class="rounded-lg bg-brand-50 px-3 py-2.5 flex items-center justify-between text-[13px]">
          <span class="text-brand-600">Pemakaian: {{ pemakaian }} kWh</span>
          <span class="font-semibold text-brand-600">Total: {{ rupiah(estimasiTotal) }}</span>
        </div>
