...
watch(
  ...
  (isOpen) => {
    if (isOpen) {
      ...
      form.tarif = props.editing?.tarif ?? 5000
    ...
    <div class="relative bg-white rounded-card shadow-card w-full max-w-md p-5 lg:p-6 my-auto">
      <div class="flex items-center justify-between mb-5">
        <h2 class="text-[16px] font-semibold text-ink-900">
          {{ editing ? 'Ubah Tagihan Air' : 'Tambah Tagihan Air' }}
        </h2>
      ...
      <form class="space-y-4" @submit.prevent="handleSubmit">
        ...
        <div>
          <label class="block text-[13px] font-medium text-ink-700 mb-1.5">Tarif per m³ (Rp)</label>
          <input
            v-model="form.tarif"
            type="number"
            min="0"
        ...
        <div class="rounded-lg bg-brand-50 px-3 py-2.5 flex items-center justify-between text-[13px]">
          <span class="text-brand-600">Pemakaian: {{ pemakaian }} m³</span>
