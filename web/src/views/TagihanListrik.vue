...
import TagihanListrikFormModal from '../components/TagihanListrikFormModal.vue'
...
async function muatData() {
  ...
  errorMsg.value = ''
  try {
    const res = await fetch('/api/tagihan/listrik')
...
async function simpan(form) {
  ...
  try {
    ...
    const url = isEdit ? `/api/tagihan/listrik/${editingItem.value.id}` : '/api/tagihan/listrik'
...
async function hapus(item) {
  if (!confirm(`Hapus tagihan listrik kamar ${item.nomor_kamar} periode ${formatPeriode(item.periode)}?`)) return
  deletingId.value = item.id
  try {
    const res = await fetch(`/api/tagihan/listrik/${item.id}`, { method: 'DELETE' })
...
async function tandaiLunas(item) {
  if (!confirm(`Tandai tagihan listrik kamar ${item.nomor_kamar} periode ${formatPeriode(item.periode)} sebagai Lunas?`)) return
  markingId.value = item.id
  try {
    const res = await fetch(`/api/tagihan/listrik/${item.id}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
...
<template>
  <div>
    ...
    <div v-else-if="daftar.length === 0" class="bg-white rounded-card border border-ink-100 shadow-card p-10 text-center max-w-md mx-auto mt-6">
      ...
      <p class="text-[14px] font-semibold text-ink-900">Belum ada tagihan listrik</p>
      <p class="text-[13px] text-ink-500 mt-1.5">Catat meter awal & akhir tiap kamar untuk menghitung tagihan listrik bulanan.</p>
        ...
        <table class="w-full text-[13.5px] min-w-[880px]">
          ...
          <tbody>
            ...
            >
              ...
              <td class="py-3 px-4 text-right text-ink-700">{{ item.pemakaian }} kWh</td>
