...
import TagihanInternetFormModal from '../components/TagihanInternetFormModal.vue'
...
async function muatData() {
  ...
  errorMsg.value = ''
  try {
    const res = await fetch('/api/tagihan/internet')
...
async function simpan(form) {
  ...
  try {
    ...
    const url = isEdit ? `/api/tagihan/internet/${editingItem.value.id}` : '/api/tagihan/internet'
...
async function hapus(item) {
  if (!confirm(`Hapus tagihan internet kamar ${item.nomor_kamar} periode ${formatPeriode(item.periode)}?`)) return
  deletingId.value = item.id
  try {
    const res = await fetch(`/api/tagihan/internet/${item.id}`, { method: 'DELETE' })
...
async function tandaiLunas(item) {
  if (!confirm(`Tandai tagihan internet kamar ${item.nomor_kamar} periode ${formatPeriode(item.periode)} sebagai Lunas?`)) return
  markingId.value = item.id
  try {
    const res = await fetch(`/api/tagihan/internet/${item.id}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
  ...
  <div>
    ...
    <div v-else-if="daftar.length === 0" class="bg-white rounded-card border border-ink-100 shadow-card p-10 text-center max-w-md mx-auto mt-6">
      <div class="w-12 h-12 rounded-xl bg-brand-50 text-brand-500 flex items-center justify-center mx-auto mb-4">
        <NavIcon name="wifi" :size="22" />
      </div>
      <p class="text-[14px] font-semibold text-ink-900">Belum ada tagihan internet</p>
      <p class="text-[13px] text-ink-500 mt-1.5">Catat provider, paket, dan biaya bulanan tiap kamar.</p>
        ...
        <table class="w-full text-[13.5px] min-w-[820px]">
          <thead>
            <tr class="text-left text-ink-500 text-[12px] border-b border-ink-100 bg-ink-100/30">
              ...
              <th class="py-3 px-4 font-medium">Paket</th>
          ...
          <tbody>
            ...
            >
              ...
              <td class="py-3 px-4 text-ink-700 whitespace-nowrap">{{ item.provider }}</td>
              <td class="py-3 px-4 text-ink-700 whitespace-nowrap">{{ item.paket }}</td>
              <td class="py-3 px-4 text-ink-700 whitespace-nowrap">{{ formatPeriode(item.periode) }}</td>
             <td class="py-3 px-4 text-right font-semibold text-ink-900 whitespace-nowrap">{{ rupiah(item.tagihan) }}</td>
